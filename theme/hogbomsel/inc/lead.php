 ...
 * or photo. They call ho_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Högboms El AB.
 *
 ...
 * @return string
 */
function ho_sample_note() {
	return ho_lead( 'ar_riktig' ) ? '' : ' <span class="ho-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ho_lead_img().
 */
function ho_lead_bild( $i ) {
	$bilder = (array) ho_lead( 'bilder', array() );
 ...
 * @return string
 */
function ho_ring_label() {
	$namn = ho_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ho_lead( 'telefon' ) ) : 'Ring ' . ho_lead( 'telefon' );
}
