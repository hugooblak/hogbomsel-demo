...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'hogbomsel-leads' ),
					'singular_name' => __( 'Lead', 'hogbomsel-leads' ),
					'edit_item'     => __( 'Förfrågan', 'hogbomsel-leads' ),
					'search_items'  => __( 'Search leads', 'hogbomsel-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'hogbomsel-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'hogbomsel-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'hogbomsel-leads' ),
			'npl_phone'    => __( 'Phone', 'hogbomsel-leads' ),
			'npl_service'  => __( 'Needs', 'hogbomsel-leads' ),
			'npl_timeline' => __( 'When', 'hogbomsel-leads' ),
			'npl_zip'      => __( 'ZIP', 'hogbomsel-leads' ),
			'npl_source'   => __( 'Source', 'hogbomsel-leads' ),
			'npl_status'   => __( 'Status', 'hogbomsel-leads' ),
			'date'         => __( 'Received', 'hogbomsel-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'hogbomsel-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'hogbomsel-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'hogbomsel-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'hogbomsel-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'hogbomsel-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'hogbomsel-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'hogbomsel-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'hogbomsel-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'hogbomsel-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'hogbomsel-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'hogbomsel-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'hogbomsel-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
