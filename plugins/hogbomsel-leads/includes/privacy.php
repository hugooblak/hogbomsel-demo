...
add_filter(
	...
	function ( $exporters ) {
		$exporters['hogbomsel-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'hogbomsel-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'hogbomsel-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'hogbomsel-leads',
						'group_label' => __( 'Quote requests', 'hogbomsel-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['hogbomsel-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'hogbomsel-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
